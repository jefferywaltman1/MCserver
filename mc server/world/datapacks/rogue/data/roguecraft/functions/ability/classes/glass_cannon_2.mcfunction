...
execute store result storage minecraft:master temp_health int 1 run scoreboard players get @e[type=marker,tag=master,limit=1] temp_health
function roguecraft:ability/classes/remove_health with storage minecraft:master
execute store result score @s current_health run attribute @s minecraft:generic.max_health base get

data merge storage minecraft:master {temp_score_level:0}
scoreboard players set @e[type=marker,tag=master,limit=1] temp_health 0
function roguecraft:ability/classes/glass_cannon_3 with storage minecraft:master

scoreboard players add @e[type=marker,tag=master,limit=1] temp_health 1
tellraw @s[scores={toggle_ability_feedback=0}] ["",{"translate":"roguecraft.chat_messages.use_ability","italic":true,"color":"white"},{"text":" "},{"translate":"roguecraft.class_ability.glass_cannon","italic":true,"color":"white"},{"text":" (","color":"white","italic":true},{"translate":"roguecraft.generic.strength","italic":true,"color":"white"},{"text":" "},{"score":{"name":"@e[type=marker,tag=master,limit=1]","objective":"temp_health"},"color":"white"},{"text":")","italic":true,"color":"white"}]
