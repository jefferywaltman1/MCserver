...
execute as @s if data storage minecraft:master {stronghold:0b} run function roguecraft:compass_nether_fortress with entity @e[type=minecraft:armor_stand,tag=structure_stand,limit=1] HandItems[0].tag.Decorations[0]
kill @e[type=armor_stand,tag=structure_stand]
execute as @s if data storage minecraft:master {stronghold:1b} run function roguecraft:compass_stronghold with storage minecraft:master Region
